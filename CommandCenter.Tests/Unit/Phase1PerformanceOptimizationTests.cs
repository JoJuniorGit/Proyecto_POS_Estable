...
public class Phase1PerformanceOptimizationTests
{
    ...
    public async Task GetProductBySkuAsync_BeforeVsAfterPerformance_ExhibitsSpeedup()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            var p = await service.GetProductQuickInfoAsync("750100000001", useCache: false);
            Assert.NotNull(p);
        }
        swBefore.Stop();
        ...
        await service.GetProductQuickInfoAsync("750100000001", useCache: true);

        var swAfter = Stopwatch.StartNew();
        for (int i = 0; i < 50; i++)
        {
            var p = await service.GetProductQuickInfoAsync("750100000001", useCache: true);
            Assert.NotNull(p);
        }
        swAfter.Stop();
    ...
    public async Task GetProductBySkuAsync_ConcurrencyTest_ExecutesSingleFactoryLookup()
    {
        ...
        var tasks = Enumerable.Range(0, 10).Select(_ => service.GetProductQuickInfoAsync("750100000999", useCache: true));
    ...
    public async Task UpdateStockAsync_InvalidatesSkuCache_Immediately()
    {
        ...
        var cached1 = await service.GetProductQuickInfoAsync("750100000555", useCache: true);
        ...
        var cached2 = await service.GetProductQuickInfoAsync("750100000555", useCache: true);
    ...
    public async Task SetProductStatusAsync_DeactivateOrDelete_InvalidatesCache()
    {
        ...
        // Popular caché
        var p1 = await service.GetProductQuickInfoAsync("750100000777", useCache: true);
        ...
        // Consultar con caché
        var p2 = await service.GetProductQuickInfoAsync("750100000777", useCache: true);
