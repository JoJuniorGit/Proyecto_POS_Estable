...
if ([string]::IsNullOrWhiteSpace($resolvedFromSecrets)) {
...
# SANs: localhost y nombre del equipo (DNS) + IPs IPv4 de la red local (iPAddress)
$sanDns = @("localhost", $env:COMPUTERNAME)
$sanIps = @()
try {
    $sanIps = @(Get-NetIPAddress -AddressFamily IPv4 -ErrorAction Stop |
        Where-Object {
...
$sanEntries = @($sanDns | ForEach-Object { "DNS=$_" }) + @($sanIps | ForEach-Object { "IPAddress=$_" })
...
Write-Host "Generando certificado autofirmado con SANs: $((@($sanDns) + @($sanIps)) -join ', ')" -ForegroundColor Cyan
...
Export-Certificate -Cert $cert -FilePath (Join-Path $certDir "pos-https.cer") -Force | Out-Null
...
Write-Host "Certificado público para instalar en el teléfono: $(Join-Path $certDir 'pos-https.cer')" -ForegroundColor Green
