...
public partial class ProductsController
{
    ...
    public async Task<IActionResult> BulkImportAsync([FromBody] Core.DTOs.BulkImportRequestDto request, System.Threading.CancellationToken cancellationToken = default)
    {
        if (!_currentUserService.CanMutateCatalog)
        {
            return this.ApiForbidden("El rol Cajero no tiene permisos para realizar importaciones.");
        }
        const int maxBatch = 5000;
        if (request == null || request.Products == null || request.Products.Count == 0)
        {
            return this.ApiBadRequest("La solicitud de importación no contiene productos.");
        }
        if (request.Products.Count > maxBatch)
        {
            return this.ApiBadRequest($"El lote de importación excede el máximo permitido ({maxBatch} productos).");
        }
        try
        {
            var result = await _inventoryService.BulkImportProductsAsync(request.Products, request.OverwriteMerge, cancellationToken);
            return Ok(new Core.DTOs.BulkImportResponseDto { Added = result.added, Updated = result.updated });
        }
    ...
    public async Task<IActionResult> ExportProductsAsync([FromQuery] string format = "xlsx", [FromQuery] bool activeOnly = true, [FromQuery] string? filter = null, System.Threading.CancellationToken cancellationToken = default)
    {
        if (!_currentUserService.CanMutateCatalog)
        {
            return this.ApiForbidden("El rol Cajero no tiene permisos para exportar el catálogo de productos.");
        }
        try
        {
    ...
    public async Task<IActionResult> ExportTemplateAsync([FromQuery] string format = "xlsx", System.Threading.CancellationToken cancellationToken = default)
    {
        if (!_currentUserService.CanMutateCatalog)
        {
            return this.ApiForbidden("El rol Cajero no tiene permisos para descargar la plantilla de importación.");
        }
        try
        {
